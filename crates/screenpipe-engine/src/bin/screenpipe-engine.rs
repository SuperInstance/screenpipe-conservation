...
use screenpipe_engine::{
    ...
    watch_pid, ConservationState, ResourceMonitor, SCServer,
};
...
async fn main() -> anyhow::Result<()> {
    ...
    // Initialize conservation state for resource-aware operation.
    // This monitors CPU and memory usage via one-sided conservation laws
    // and adjusts recording quality to prevent system overload.
    let conservation_state = ConservationState::new();
    let conservation_state_clone = conservation_state.clone();
    start_conservation_monitor(conservation_state_clone, Duration::from_secs(10));

    let resource_monitor = ResourceMonitor::new(
        config.analytics_enabled,
        Some(conservation_state.clone()),
    );
