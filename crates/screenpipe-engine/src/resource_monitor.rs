...
pub struct ResourceMonitor {
    ...
    /// Conservation state for resource-aware operation
    conservation_state: Option<Arc<ConservationState>>,
}
...
impl ResourceMonitor {
    pub fn new(telemetry_enabled: bool, conservation_state: Option<Arc<ConservationState>>) -> Arc<Self> {
            ...
            loop {
                tokio::select! {
                    _ = tokio::time::sleep(interval) => {
                        ...
                        // Feed metrics into conservation checker
                        if let Some(ref cs) = monitor.conservation_state {
                            let pid = std::process::id();
                            ...
                            if let Some(process) = sys.process(pid_sys) {
                                ...
                                for child in sys.processes().values() {
                                    if child.parent() == Some(pid_sys) {
                                        total_cpu += child.cpu_usage();
                                        total_mem_mb += child.memory() as f64 / (1024.0 * 1024.0);
                                    }
                                }
                            }
                            cs.update_metrics(total_cpu as f64, total_mem_mb);
                        }
