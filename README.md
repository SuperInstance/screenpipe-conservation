...
6. **Resource conservation**: A background task tracks process-tree CPU and memory usage and feeds them into a conservation-checker instance. Phase detection enables proactive quality degradation before resource exhaustion.
7. **UI layer**: Desktop app built with Tauri (Rust + TypeScript).
...
|------|-----------------|--------------|-------------|-------------------|
| **Full** | Normal | ✅ Yes | 100% | ✅ Yes |
| **Reduced** | 2× interval | ❌ Skipped | 70% | ✅ Yes |
...
**How does screenpipe handle resource pressure?**
screenpipe includes integrated resource conservation. It uses one-sided conservation laws to track CPU and memory headroom, and automatically degrades recording quality when resources are scarce. This means screenpipe can run 24/7 without crashing your machine — it adapts to your workload instead of competing with it. See [Resource Conservation](#resource-conservation) for details.
